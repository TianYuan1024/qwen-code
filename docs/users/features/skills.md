...
- Resume does not re-apply them for a Skill a fresh tool call would refuse today (disabled, hidden from the model with `disable-model-invocation`, or a `paths:` Skill not yet activated), for a **project** Skill whose folder is no longer trusted, or when the recorded response cannot be matched against the Skill's `SKILL.md` on disk — for example because the file was edited, the checkout moved, or the body was truncated for the model. When a Skill that declares `hooks:` or `allowedTools` is skipped this way, the debug log says why.
